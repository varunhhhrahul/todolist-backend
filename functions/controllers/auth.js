...
exports.getMe = asyncHandler(async (req, res, next) => {
  try {
    ...
    const user = await User.findById(req.user._id).populate(
      'following.user followers.user posts messageRequest.user messageRequest.sentBy posts.user'
    );
    res.status(200).json({
      success: true,
      data: user,
...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  // Create reset url
  const resetUrl = `https://gamers-world-d45d7.web.app/auth/resetpassword/${resetToken}`;
